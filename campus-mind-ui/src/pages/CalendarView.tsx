...
const CalendarView = () => {
  ...
  const events: CalendarEvent[] = [
    { id: "1", title: "CS 101 Assignment", date: new Date(2025, 9, 13), type: "assignment", course: "CS 101", time: "11:59 PM" },
    { id: "2", title: "Math Quiz", date: new Date(2025, 9, 14), type: "quiz", course: "MATH 201", time: "2:00 PM" },
    { id: "3", title: "Physics Midterm", date: new Date(2025, 9, 15), type: "exam", course: "PHYS 150", time: "10:00 AM" },
    { id: "4", title: "Team Meeting", date: new Date(2025, 9, 15), type: "event", time: "3:00 PM" },
    { id: "5", title: "Lab Report Due", date: new Date(2025, 9, 16), type: "assignment", course: "CHEM 101", time: "11:59 PM" },
    { id: "6", title: "Study Group", date: new Date(2025, 9, 17), type: "event", time: "6:00 PM" },
  ];
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex rounded-full border-2 border-foreground p-1">
            <Button
              ...
              className={`rounded-full px-6 ${viewMode === "week" ? "bg-primary text-primary-foreground hover:bg-primary hover:text-primary-foreground" : "hover:bg-transparent"}`}
            >
              Week
            </Button>
            <Button
              ...
              className={`rounded-full px-6 ${viewMode === "month" ? "bg-primary text-primary-foreground hover:bg-primary hover:text-primary-foreground" : "hover:bg-transparent"}`}
            >
              Month
            </Button>
          </div>
            ...
            {getWeekDays().map((day) => {
              ...
              return (
                <div key={day.toString()}>
                  {/* Day Header */}
                  <div className={`mb-3 rounded-3xl p-4 text-center border-2 border-foreground ${
                    isToday ? "bg-primary text-primary-foreground" : "bg-muted"
                  }`}>
                    <div className="text-sm font-semibold mb-1">{format(day, "EEE")}</div>
                  ...
                  <div className="space-y-2">
                    {dayEvents.map((event) => (
                      <div
                        key={event.id}
                        className={`rounded-2xl border-2 border-foreground p-3 shadow-card ${getTypeColor(event.type)}`}
                      >
                        <p className="font-bold text-sm mb-1">{event.title}</p>
                        <p className="text-xs font-medium">{event.time}</p>
                        {event.course && (
                          <p className="text-xs font-medium mt-0.5">{event.course}</p>
                        )}
                      </div>
                    ))}
                  </div>
          ...
          <Card className="border-2 border-foreground bg-background p-6 rounded-3xl shadow-card">
            ...
            <div className="grid grid-cols-7 gap-3">
              ...
              {getMonthDays().map((day, index) => {
                if (!day) {
                  return <div key={`empty-${index}`} className="aspect-square rounded-3xl border-2 border-foreground bg-background" />;
                }
                    ...
                    <div className="space-y-1 text-[10px]">
                      {dayEvents.slice(0, 2).map((event) => {
                        const eventColor = 
                          ...
                          event.type === "quiz" ? "text-purple-600" :
                        ...
                        return (
                          ...
                          >
                            <div className={`h-1.5 w-1.5 rounded-full flex-shrink-0 mt-0.5 ${getEventDotColor(event.type)}`} />
                            <span className={`truncate font-semibold leading-tight ${eventColor}`}>
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1">
                        ...
                        {event.time && (
                          <p className="mt-0.5 text-sm text-muted-foreground">{event.time}</p>
                        )}
                      </div>
